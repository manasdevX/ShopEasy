...
export default function Cart() {
  ...
  // New Calculation: Total MRP
  const totalMRP = cartItems.reduce(
    (acc, item) =>
      acc + (item.product?.mrp || item.product?.price || 0) * item.quantity,
    0
  );

  const discount = totalMRP - subtotal; // This is your "You Saved" amount

  const FREE_SHIPPING_THRESHOLD = 400;
  // ... (keep the rest of your existing logic)P
      ...
      <main className="flex-grow max-w-7xl mx-auto w-full px-6 py-12">
        ...
        {cartItems.length > 0 ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-1">
              <div className="bg-white dark:bg-slate-900 border border-slate-100 dark:border-slate-800 rounded-3xl p-8 sticky top-28 shadow-sm">
                <h2 className="text-xl font-black tracking-tighter text-slate-900 dark:text-white mb-6 uppercase">
                  Order Summary
                </h2>
          ...
          <div className="text-center py-24 bg-white dark:bg-slate-900 rounded-3xl border border-slate-100 dark:border-slate-800">
            ...
            <Link
              to="/"
              className="mt-8 bg-slate-900 dark:bg-white dark:text-slate-900 text-white px-8 py-3 rounded-xl font-bold inline-block"
            >
              Start Shopping
            </Link>
          </div>
