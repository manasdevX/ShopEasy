...
import { ArrowRight, Eye, EyeOff, ShieldCheck, Loader2, CheckCircle2, AlertCircle } from "lucide-react";
import { useNavigate } from "react-router-dom";
import SellerNavbar from "../../components/Seller/SellerNavbar";
import SellerFooter from "../../components/Seller/SellerFooter";
// Import your custom toast utilities
...
export default function BankDetails() {
  ...
  const [ifscInfo, setIfscInfo] = useState({ bank: "", branch: "", loading: false, error: false });
  ...
  // Effect to verify IFSC Code when 11 characters are typed
  useEffect(() => {
    const verifyIFSC = async () => {
      if (bankData.ifscCode.length === 11) {
        setIfscInfo((prev) => ({ ...prev, loading: true, error: false }));
        try {
          const res = await fetch(`https://ifsc.razorpay.com/${bankData.ifscCode}`);
          if (res.ok) {
            const data = await res.json();
            setIfscInfo({ bank: data.BANK, branch: data.BRANCH, loading: false, error: false });
            // Optional: notify user the IFSC is valid
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      // Simulate Backend Verification (Penny Drop)
      await new Promise((resolve) => setTimeout(resolve, 2000));
      
      showSuccess("Verification Successful! ₹1.00 deposited.");
      navigate("/Seller/Dashboard");
    } catch (err) {
      showError("Bank verification failed. Please try again later.");
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="flex-grow flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-8 border dark:border-slate-800">
          <div className="text-center mb-10">
            <h1 className="text-4xl font-black text-orange-500 tracking-tight mb-2">ShopEasy</h1>
            <p className="text-center text-gray-500 dark:text-slate-400 mb-8 font-medium">Final step to start receiving payments</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="relative">
              <input
                ...
                onPaste={(e) => {
                  e.preventDefault();
                  showError("Pasting is disabled for security. Please type the number.");
                }}
                placeholder="Confirm Account Number"
                className={`${inputBase} ${confirmAccountNumber && confirmAccountNumber !== bankData.accountNumber ? 'border-red-500' : ''}`}
            ...
            <div className="space-y-2">
              <div className="relative">
                ...
                />
                {ifscInfo.loading && (
                   <Loader2 className="absolute right-5 top-1/2 -translate-y-1/2 animate-spin text-orange-500" size={18} />
                )}
              </div>
              ...
              {ifscInfo.bank && (
                <div className="mx-1 px-3 py-2 bg-green-500/10 rounded-lg border border-green-500/20">
                  ...
                  <p className="text-[9px] text-green-600/80 font-medium">Branch: {ifscInfo.branch}</p>
                </div>
              )}
            </div>
            ...
            <div className="bg-slate-50 dark:bg-[#161f35]/50 border border-slate-100 dark:border-slate-800 rounded-lg p-4 flex gap-3">
              ...
              <p className="text-[11px] text-slate-500 dark:text-slate-400 font-medium leading-relaxed">
                We'll verify your account with a small ₹1 deposit. This ensures your payments reach you safely.
              </p>
            </div>
