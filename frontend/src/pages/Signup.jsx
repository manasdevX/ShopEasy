...
export default function Signup() {
  ...
  const sendEmailOtp = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.success(isEmailSent ? "OTP Resent to Email!" : "OTP sent to Email!");
      } else {
  ...
  const sendMobileOtp = async () => {
    ...
    if (isMobileVerified) return toast.success("Phone already verified!");

    setVerifyingMobile(true);
    setPhoneError("");

    try {
      const res = await fetch("http://localhost:5000/api/auth/send-mobile-otp", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      if (res.ok) {
        ...
        toast.success(isMobileSent ? "OTP Resent to Mobile!" : "OTP sent to Mobile!");
      } else {
        setPhoneError(data.message);
        toast.error(data.message);
      }
    } catch {
      toast.error("Server Error while sending mobile OTP");
    } finally {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!isEmailVerified) return toast.error("Please verify your email address first");
    if (!isMobileVerified) return toast.error("Please verify your phone number first");
    ...
    try {
      ...
      if (res.ok) {
        ...
        // TOAST: Success
        toast.success("Welcome to ShopEasy! Registration complete.");
        navigate("/");
      } else {
    ...
    <div className="min-h-screen flex flex-col bg-gray-50 dark:bg-[#030712] transition-colors duration-300 font-sans">
      ...
      <div className="flex-grow flex items-center justify-center px-4 py-12">
        <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-2xl shadow-xl p-8 border dark:border-slate-800">
          ...
          <p className="text-center text-gray-500 dark:text-slate-400 mb-8 font-medium">Create your account</p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-3">
              <div className="relative flex items-center">
                <input
                  ...
                  className={`${inputBase} pr-24 ${
                    isEmailVerified ? "border-green-500 ring-1 ring-green-500/20" : "focus:ring-2 focus:ring-orange-400"
                  }`}
                  required
                />
                <div className="absolute right-2 flex items-center">
                  {!isEmailVerified ? (
                    ...
                    >
                      {verifyingEmail ? <Loader2 className="animate-spin" size={16} /> : emailTimer > 0 ? `${emailTimer}s` : isEmailSent ? "Resend" : "Verify"}
                    </button>
                  ) : <CheckCircle className="text-green-500 mr-2" size={20} />}
                </div>
              </div>
              {emailError && <p className="text-red-500 text-xs font-medium ml-1">{emailError}</p>}
              {isEmailSent && !isEmailVerified && !googleId && (
                <div className="flex gap-2 p-2 bg-gray-50 dark:bg-slate-800/50 rounded-lg border border-dashed dark:border-slate-700 animate-in zoom-in-95">
                  ...
                  />
                  <button type="button" onClick={verifyEmailOtp} className="bg-green-600 text-white px-4 py-1.5 rounded-md text-xs font-bold hover:bg-green-700 transition-colors">CONFIRM</button>
                </div>
              )}
            </div>
            ...
            <div className="space-y-3">
              <div className="relative flex items-center">
                <input
                  ...
                  className={`${inputBase} pr-24 ${
                    isMobileVerified ? "border-green-500 ring-1 ring-green-500/20" : "focus:ring-2 focus:ring-orange-400"
                  }`}
                  required
                />
                <div className="absolute right-2 flex items-center">
                  {!isMobileVerified ? (
                    <button
                      ...
                      className="text-xs font-bold text-orange-500 hover:text-orange-600 disabled:text-gray-400 uppercase tracking-wider px-2"
                    >
                      {verifyingMobile ? <Loader2 className="animate-spin" size={16} /> : mobileTimer > 0 ? `${mobileTimer}s` : isMobileSent ? "Resend" : "Verify"}
                    </button>
                  ) : <CheckCircle className="text-green-500 mr-2" size={20} />}
                </div>
              </div>
              {phoneError && <p className="text-red-500 text-xs font-medium ml-1">{phoneError}</p>}
              {isMobileSent && !isMobileVerified && (
                <div className="flex gap-2 p-2 bg-gray-50 dark:bg-slate-800/50 rounded-lg border border-dashed dark:border-slate-700 animate-in zoom-in-95">
                  <input
                    ...
                    className="flex-grow bg-white dark:bg-slate-800 border dark:border-slate-700 px-3 py-1.5 rounded-md text-sm outline-none focus:ring-2 focus:ring-green-400"
                  />
                  <button type="button" onClick={verifyMobileOtp} className="bg-green-600 text-white px-4 py-1.5 rounded-md text-xs font-bold hover:bg-green-700 transition-colors">CONFIRM</button>
                </div>
              )}
            </div>
            ...
            >
              {loading ? <span className="flex items-center justify-center gap-2"><Loader2 className="animate-spin" size={20} /> Processing...</span> : "Create Account"}
            </button>
          </form>
          ...
          <p className="mt-8 text-center text-sm text-gray-500 dark:text-slate-400">
            Already have an account? <Link to="/login" className="text-orange-500 hover:text-orange-600 font-bold transition-colors">Login</Link>
          </p>
        </div>
      </div>
      <AuthFooter />
